...
function fieldHelp(string $name,string $label): string {
    ...
    $tips=[
      ...
      'country'=>'Выберите страну покупателей вашего оффера. В API отправляется код ISO, например ZA для ЮАР. Выбор страны не создаёт и не заменяет IP покупателя. Для Skylead/Cashfactories рекомендуется указать явно, чтобы ПП не определяла страну по IP. Для Lemonad поле не используется.',
